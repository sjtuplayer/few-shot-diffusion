...
class Attention(nn.Module):
    def __init__(self, dim,dim_out=0, heads = 4, dim_head = 32):
        super().__init__()
        if dim_out==0:
            dim_out=dim
        self.scale = dim_head ** -0.5
...
class content_fusion(nn.Module):
    ...
    def forward(self,x,condition):
        b,c,h, w = x.size()
        y=self.convs(torch.cat([x,condition],dim=1))
        return y
class MyUnet(Unet):
    def prepare(self,dim=64,
        ...
        if style_condition:
            ...
            self.c_content_condition=content_fusion()
...
#                 t2 = c_time_mlp(time)
#                 x_content=c_conv(x_self_cond['content'])
#                 x_content=c_block1(x_content,t2)
#                 x_content=c_block2(x_content,t2)
#                 x_content=c_attn(x_content)
#                 if x_content.size(-1)!=x.size(-1):
#                     print(x_content.shape,x.shape)
#                     x_content=self.resize(x_content)
#                 x=x+x_content
...
#         x = self.mid_attn(x)
#         x = self.mid_block2(x, t)
#         # if self.style_condition and x_self_cond is not None:
#         #     x=self.c_style_condition(x,time,x_self_cond)
